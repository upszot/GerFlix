...
{
    printf("\n ID %d - Nombre: %s",serie.idSerie,serie.nombre);
...
int get_id_Serie(eSerie series[], char * Nombre_Serie, int cant)
{
    ...
    for(int i =0;((i>cant) || (ID!=-1));i++)
    {
        if(series[i].estado!=0)
        {
                printf("ID: %d\n ",ID);
                system("pause");
             //if(strcmp(NewUser.nombre, "") == 0)
            if(strcmp(series[i].nombre,Nombre_Serie)==0)
            {//Encontro la serie Buscada
                ID=series[i].idSerie;
            }
        }
    }
